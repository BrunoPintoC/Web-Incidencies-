<?php
    ...
    if ($operacio!="new" && $operacio!="edit") header("Location: list_incidencies.php");
    obrirConnexioBD();
    if ($operacio=="edit") {
        if (isset($_REQUEST["id_customer"])) {
            $id_customer=$_REQUEST["id_customer"];
            $sql = "SELECT * FROM incidencies WHERE id_customer=" . $id_customer;
            $result = $conn->query($sql);
            if ($result->num_rows == 0) {
                tancarConnexioBD();
                header("Location: list_incidencies.php?");
            } else {
	...
	<div class="container">
		...
		<div class="row myform">
			<div class="col-md-7 col-md-offset-3">
				...
				<form name="form_incidencies" action="mod_customer.php?operacio=<?=$operacio?>" role="form" method="post">
                ...
                <?php   } ?>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="tipus" id="customer">
                    ...
                    <div class="form-group">
					   <label class="control-label" for="observacions">Observacions:</label>
                        ...
                        <div class="checkbox">
					       <label><input type="checkbox" name="observacions" id="observacions" value="no usual"<?php if (isset($row)) if ($row["observacions"]=="no usual") echo " checked";?>>customer poc usual</label>
						</div>
                    </div>
                    <div class="form-group">
